...
const CURRENCY_MAP = {
  USD: { isoCode: "USD", decimalDigits: 2, locale: "en-US" },
  EUR: { isoCode: "EUR", decimalDigits: 2, locale: "fr-FR" },
  GBP: { isoCode: "GBP", decimalDigits: 2, locale: "en-GB" },
  TND: { isoCode: "TND", decimalDigits: 3, locale: "ar-TN" },
};

const resolveCurrency = (currency) => {
  if (!currency) return CURRENCY_MAP.USD;
  if (typeof currency === "object") return currency;
  const code = String(currency).toUpperCase().trim();
  return CURRENCY_MAP[code] || CURRENCY_MAP.USD;
};

const formatEmailAmount = (amount, currency) => {
                                      ...
                                      <tr>
                                        <td style='width:150px;'>
                                          ...
                                          <img
                                            alt
                                            height='auto'
                                            src='https://res.cloudinary.com/ahossain/image/upload/v1676296566/Sofiagen%20files/cc_v8jite.png'
                              ...
                              <tbody>
                                ...
                                <tr>
                                  ...
                                  >
                                   ...
                                   >
                                     ${formatEmailAmount(item.price, option.currency)}
                                   </td>
                                   <td
                                     id='common_table'
                                     style='padding: 2px 4px; text-align:end'
                                   >
                                     ${formatEmailAmount(item.price * item.quantity, option.currency)}
                                   </td>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td style='padding: 2px 4px;text-align:justify; font-size:13px;'>
                          ${formatEmailAmount(option.subTotal, option.currency)}
                        </td>
                   
                        <td style='padding: 2px 4px;text-align:justify; font-size:13px;'>
                        ${formatEmailAmount(option.shipping, option.currency)}
                      </td>
                        <td style='padding: 2px 4px;text-align:justify; font-size:13px;'>
                          ${formatEmailAmount(option.discount, option.currency)}
                        </td>
                        <td style='padding: 2px 4px;text-align:justify;color:red ; font-size:13px;'>
                          ${formatEmailAmount(option.total, option.currency)}
                        </td>
