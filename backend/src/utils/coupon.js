const coupons = [
  {
    status: "show",
    title: {
      en: "Summer Gift Voucher",
      de: "sommer geschenkgutschein",
      hy: "ata|aukv v~eé - ~axéyé",
      af: "somer geskenkbewys",
      cs: "letné dérkov poukaz",
      bn: "Ͱͮǰ  ",
      az: "yay hYdiyyY vauéri",
      ln: "sanza ya Zomi na Mibale",
    },
    couponCode: "SUMMER26",
    endTime: "2026-10-18T20:38:00.000Z",
    minimumAmount: 500,
    productType: "Grocery",
    logo: "https://i.ibb.co/23kQcB9/ins3.jpg",
    discountType: {
      type: "percentage",
      value: 10,
    },
  },
  {
    status: "show",
    title: {
      en: "Winter Gift Voucher",
      de: "wintergeschenkgutschein",
      az: "q1_ hYdiyyY vauéri",
      ln: "zima Mpho Mpho Mpho",
      ar: "B3JE) G/'J' 'D4*'!",
      pt: "voucher de presente de inverno",
    },
    couponCode: "WINTER25",
    endTime: "2025-05-31T20:19:00.000Z",
    minimumAmount: 1500,
    productType: "Grocery",
    logo: "https://i.ibb.co/wBBYm7j/ins4.jpg",
    discountType: {
      type: "fixed",
      value: 100,
    },
  },
  {
    status: "show",
    title: {
      en: "Summer Gift Voucher",
      de: "sommer geschenkgutschein",
    },
    couponCode: "SUMMER24",
    endTime: "2024-12-20T00:56:00.000Z",
    minimumAmount: 1000,
    productType: "Cloths",
    logo: "https://i.ibb.co/4thS4Z1/ins2.jpg",
    discountType: {
      type: "percentage",
      value: 10,
    },
  },
  {
    status: "show",
    title: {
      en: "August Gift Voucher",
      de: "august geschenkgutschein",
      hy: "écx}x}uav v~eé ~axéyé",
      af: "augustus geskenkbewys",
      cs: "augustové dérkov poukaz",
      az: "avqust hYdiyyY vauéri",
      ln: "sénzé ya zémi na mTkT",
      ar: "B3JE) G/'J' #:373",
      pt: "voucher presente agosto",
    },
    couponCode: "AUGUST24",
    endTime: "2024-10-31T08:30:00.000Z",
    minimumAmount: 2000,
    productType: "Grocery",
    logo: "https://i.ibb.co/PDLPDHc/ins1.jpg",
    discountType: {
      type: "percentage",
      value: 50,
    },
  },
];
module.exports = coupons;
