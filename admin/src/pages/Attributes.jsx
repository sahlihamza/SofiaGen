...
const Attributes = () => {
  ...
  const canCreateAttribute = hasPermission("attributes", "create");
  ...
  // GET /api/attributes/ now returns a paginated object { attributes, totalDoc, ... }
  const attributeList = data?.attributes || [];

  const { handleDeleteMany, allId } = useToggleDrawer();
  ...
  const handleSelectAll = () => {
    ...
    setIsCheck(attributeList.map((value) => value._id));
    if (isCheckAll) {
      setIsCheck([]);
    }
  };
