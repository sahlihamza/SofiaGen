...
const InvoiceForPrint = ({ data, printRef, globalSetting, currency }) => {
  const { t } = useTranslation();

  const currencyObj = currency || { isoCode: "USD", decimalDigits: 2, locale: "en-US" };
                    ...
                    >
                      ...
                      <TableCell className="text-right py-1">
                        <span className="text-right font-bold text-gray-700 bill">
                          {" "}
                          {formatMoney(item.price * item.quantity, currencyObj)}
                        </span>
                      </TableCell>
                    </TableRow>
                  ))}
                    ...
                    <p className="bill">
                      ...
                      {or?.paymentDetails?.selectPaymentOption_Card !==
                        ...
                        <span className="text-xs bill">
                          ...
                          <span className="font-semibold text-gray-900">
                            {formatMoney(parseFloat(or?.paymentDetails?.paymentAmount_Card), currencyObj)}
                          </span>
                        </span>
                      )}
                      <br />
                      {or?.paymentDetails?.selectPaymentOption_Cash !==
                        ...
                        <span className="text-xs bill">
                          {or?.paymentDetails?.selectPaymentOption_Cash}:{" "}
                          <span className="font-semibold text-gray-900">
                            {formatMoney(parseFloat(or?.paymentDetails?.paymentAmount_Cash), currencyObj)}
                          </span>
                        </span>
                      )}
                      <br />
                      {or?.paymentDetails?.selectPaymentOption_Credit !==
                        ...
                        <span className="text-xs bill">
                          {or?.paymentDetails?.selectPaymentOption_Credit}:{" "}
                          <span className="font-semibold text-gray-900">
                            {formatMoney(parseFloat(or?.paymentDetails?.paymentAmount_Credit), currencyObj)}
                          </span>
                        </span>
                      )}
                    </p>
                ...
                <div className="mt-2">
                  <h5 className="flex justify-between font-medium text-xs ">
                    ...
                    <span className="font-semibold ">
                      {formatMoney(parseFloat(or?.subTotal), currencyObj)}
                    </span>
                  </h5>
                  ...
                  {or?.shippingCost > 0 && (
                    <h5 className="flex justify-between font-medium text-xs">
                      ...
                      <span className="font-semibold ">
                        {formatMoney(parseFloat(or?.shippingCost), currencyObj)}
                      </span>
                    </h5>
                  )}
                  {or?.discount > 0 && (
                    <h5 className="flex justify-between font-medium text-xs">
                      ...
                      <span className="font-semibold">
                        {formatMoney(parseFloat(or?.discount), currencyObj)}
                      </span>
                    </h5>
                  )}
                  <h3 className="flex justify-between font-medium text-xs border-t border-black mt-2">
                    <span> {t("Total")} : </span>
                    <span className="font-semibold ">
                      {formatMoney(parseFloat(or?.total), currencyObj)}
                    </span>
                  </h3>
                </div>
              </div>
            ...
            <div className="flex justify-between -mt-3 mr-1 mb-4">
                  ...
                  <p className="bill">
                    ...
                    {data?.paymentDetails?.selectPaymentOption_Card !==
                      ...
                      <span className="text-xs bill">
                        {data?.paymentDetails?.selectPaymentOption_Card}:{" "}
                        <span className="font-semibold text-gray-900">
                          {formatMoney(parseFloat(data?.paymentDetails?.paymentAmount_Card), currencyObj)}
                        </span>
                      </span>
                    )}
                    <br />
                    {data?.paymentDetails?.selectPaymentOption_Cash !==
                      ...
                      <span className="text-xs bill">
                        {data?.paymentDetails?.selectPaymentOption_Cash}:{" "}
                        <span className="font-semibold text-gray-900">
                          {formatMoney(parseFloat(data?.paymentDetails?.paymentAmount_Cash), currencyObj)}
                        </span>
                      </span>
                    )}
                    <br />
                    {data?.paymentDetails?.selectPaymentOption_Credit !==
                      ...
                      <span className="text-xs bill">
                        {data?.paymentDetails?.selectPaymentOption_Credit}:{" "}
                        <span className="font-semibold text-gray-900">
                          {formatMoney(parseFloat(data?.paymentDetails?.paymentAmount_Credit), currencyObj)}
                        </span>
                      </span>
                    )}
                  </p>
              ...
              <div className="mt-2">
                <h5 className="flex justify-between font-medium text-xs ">
                  <span>{t("GrossTotal")} :</span>{" "}
                  <span className="font-semibold ">
                    {formatMoney(parseFloat(data?.subTotal), currencyObj)}
                  </span>
                </h5>
                ...
                {data?.shippingCost > 0 && (
                  <h5 className="flex justify-between font-medium text-xs">
                    <span> {t("ShippingCostLower")} :</span>{" "}
                    <span className="font-semibold ">
                      {formatMoney(parseFloat(data?.shippingCost), currencyObj)}
                    </span>
                  </h5>
                )}
                {data?.discount > 0 && (
                  <h5 className="flex justify-between font-medium text-xs">
                    <span> {t("DiscountLower")} :</span>{" "}
                    <span className="font-semibold">
                      {formatMoney(parseFloat(data?.discount), currencyObj)}
                    </span>
                  </h5>
                )}
                <h3 className="flex justify-between font-medium text-xs border-t border-black mt-2">
                  <span> {t("Total")} : </span>
                  <span className="font-semibold ">
                    {formatMoney(parseFloat(data?.total), currencyObj)}
                  </span>
                </h3>
              </div>
            </div>
