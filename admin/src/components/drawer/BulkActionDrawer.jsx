...
import useBulkActionSubmit, { NO_SHIPPING_CLASS } from "@/hooks/useBulkActionSubmit";
...
const BulkActionDrawer = ({
  ...
  useEffect(() => {
    if (title !== "Products" || !isBulkDrawerOpen || !ids?.length) return;
    (async () => {
      try {
        const products = await Promise.all(
          ids.map((id) => ProductServices.getProductById(id))
        );
        const classIds = products.map(
          (product) => product?.shippingClassId?._id || product?.shippingClassId || ""
        );
        const allSame = classIds.every((classId) => classId === classIds[0]);
        if (!allSame) {
          setBulkShippingClassId("");
        } else {
          setBulkShippingClassId(classIds[0] === "" ? NO_SHIPPING_CLASS : classIds[0]);
        }
      } catch {
        // leave it on "â€” No change â€”" if the current values can't be determined
      }
    })();
  }, [title, isBulkDrawerOpen, ids]);
              ...
              {title === "Products" && (
                <>
                  <div className="grid grid-cols-6 gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6">
                    ...
                    <div className="grid grid-cols-6 gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6">
                      <LabelArea label="Shipping Class" />
                      <div className="col-span-8 sm:col-span-4">
                        <Select
                          value={bulkShippingClassId}
                          onChange={(e) => setBulkShippingClassId(e.target.value)}
                          className="h-12"
                        >
                          <option value="">â€” No change â€”</option>
                          <option value={NO_SHIPPING_CLASS}>No shipping class</option>
                  ...
                  <>
                    <div className="grid grid-cols-6 gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6">
                      ...
                      <div className="col-span-8 sm:col-span-4">
                        <Input
                          {...register(`endDate`, {
                            required: "Platform Coupon End Date is required",
                          })}
                          label="Platform Coupon End Date"
                          name="endDate"
                          type="datetime-local"
