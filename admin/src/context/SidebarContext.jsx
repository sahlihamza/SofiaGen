...
const defaultInterfaceLanguages = [
  { iso_code: "en", name: "English", flag: "US" },
...
export const SidebarProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!supportedInterfaceLanguages.includes(selectedLang)) {
      selectedLang = "en";
    }
    ...
    if (currLangCookie) {
      try {
        parsed = JSON.parse(currLangCookie);
      } catch (e) {
        console.error("Error parsing _currLang cookie:", e);
      }
    }

    if (parsed && typeof parsed === "object" && parsed.iso_code) {
      setCurrLang(parsed);
    } else if (!pathname) {
      const languageOptions = languages?.length
        ? languages
        : defaultInterfaceLanguages;
      const result = languageOptions?.find(
        (lang) => lang?.iso_code === selectedLang
      );
      if (result) {
        setCurrLang(result);
